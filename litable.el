      ...
      (cond
       ...
       ((string-match "\\`let\\*?\\'" (symbol-name function))
        (dolist (cur (car rest))      ;For each item in the first form
          (when (and (listp cur) (listp (cdr cur))) ;If the variable's value is set with a function call
            (litable--deep-search-for-impures (cdr cur)))) ;Check the function call
        (dolist (cur (cdr rest))      ;Then check the actual content of the let form
          (when (listp cur) (litable--deep-search-for-impures cur))))
       ;; If it's a lambda, we can skip the first argument (it's the argument list)
       ((eq function 'lambda)
        (dolist (cur (cdr rest))
          (when (listp cur) (litable--deep-search-for-impures cur))))
       ;; Anything inside a quote is considered safe, because anyting
       ;; that could evaluate is considered unsafe.
